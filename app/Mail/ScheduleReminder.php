...
class ScheduleReminder extends Mailable
{
    use Queueable, SerializesModels;
        ...
        public function __construct($booking, $admin)
        {
            ...
            //pengaman
            $categoryName = $booking->category->name ?? 'Layanan/Promo';
            $basePrice = $booking->category->base_price ?? 0;
            
            $msg = "*Reminder H-1 Makeup* (*{$categoryName}*):
    ...
    - ⁠*lokasi ada di {$booking->address} ({$booking->person_count} orang)*. Makeup nya 1 orang Rp".number_format($basePrice,0,',','.')." dan sudah membayar dp Rp".number_format($booking->dp_amount,0,',','.').", jadi *total cash nya Rp".number_format($totalCash,0,',','.')."* yaa.
