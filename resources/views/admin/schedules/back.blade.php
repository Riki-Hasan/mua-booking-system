                            ...
                            <div class="bg-slate-50 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden border border-slate-300">
                                ...
                                <div class="summary-parent">
                                    ...
                                    <div class="s-div4 pt-6"><label class="block mb-2 font-black text-[9px] uppercase text-black">Metode</label>
                                        <select name="payment_method" id="payMethod" onchange="togglePaymentInput()" class="p-3 bg-white border-black border-4 text-black text-xs uppercase outline-none rounded-xl w-full ">
                                            <option value="cash" class=" text-black font-bold">Tunai (Cash)</option>
                                            <option value="qris" class=" text-black font-bold">QRIS / Transfer</option>
                                        </select>
                                    </div>
        ...
        function togglePaymentInput() {
            ...
            if (method === 'cash') { 
                area.innerHTML = `<label class="block text-[9px] font-black text-black uppercase mb-2 ml-1 tracking-widest ">Nominal Tunai</label>
                                  <input type="number" name="dp_amount" id="dpInput" placeholder="0" class="w-full bg-white border-black border-2 rounded-xl p-3 text-sm text-black outline-none">`; 
            } else { 
                area.innerHTML = `<label class="block text-[9px] font-black text-gray-500 uppercase mb-2 ml-1 tracking-widest">Bayar</label>
                                  <div class="select-wrapper"><select name="dp_amount" id="dpInput" class="p-3 bg-white border-black border-2 text-black rounded-xl text-xs uppercase outline-none w-full font-bold">
                                      <option value="${totalVal/2}" class="font-bold">DP 50% (Rp${new Intl.NumberFormat('id-ID').format(totalVal/2)})</option>
                                      <option value="${totalVal}" class="font-bold">Lunas (Rp${new Intl.NumberFormat('id-ID').format(totalVal)})</option>
                                  </select></div>`; 
            }
        }
